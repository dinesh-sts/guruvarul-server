                ...
                <div class="card-body">
                    <form action="{{ route('admin.smsSettingsUpdate') }}" method="POST" enctype="multipart/form-data">
			 	    ...
			 	    <select id="sms" name="activeapi" class="form-control" required>
                                        ...
                                        <option id="fast_2_sms" value="fast2sms" 
                                            @if(isset($activeapi->value)) {{$activeapi->value == "fast2sms" ? "selected" : ''}}@endif>
                                            Fast 2 SMS
                                        </option>
                                        <option id="msg_91" value="msg91" 
                                            @if(isset($activeapi->value)) {{$activeapi->value == "msg91" ? "selected" : ''}}@endif>
                                            MSG 91
                                        </option>
                                        <option id="firebase" value="firebase" 
                                            @if(isset($activeapi->value)) {{$activeapi->value == "firebase" ? "selected" : ''}}@endif>
                                            Firebase
                                        </option>
                                    </select>
                                </div>
