...
<div class="container pt-3">
    ...
    <h3 class="colorSecondary inATitle1">CMS Pages</h3>
    ...
    <div class="inMemberTopPanel">
        <div class="pb-0">
            <div class="row">
                <div class="mb-3">
                    <a href="{{route('admin.cmsCreate')}}" class="btn btnSecondary">
                        <i class="fas fa-plus pe-1"></i>Add CMS Pages
                    </a>
                    <div class="dropdown d-inline float-end ms-2 me-2">
                        ...
                        <ul class="dropdown-menu inBorderColor1">
                            <li>
                                <a href="{{route('admin.cmsList',['filter' => 'all'])}}" name="filter" value="all" class="dropdown-item">All CMS<span class="badge text-bg-secondary ms-1">@if(isset($cmspagesCount)){{$cmspagesCount}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.cmsList',['filter' => 'approved'])}}" name="filter" value="approved" class="dropdown-item">Approved<span class="badge text-bg-secondary ms-1">@if(isset($cmspagesApprovedCount)){{$cmspagesApprovedCount}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.cmsList',['filter' => 'unapproved'])}}" name="filter" value="unapproved" class="dropdown-item">Unapproved<span class="badge text-bg-secondary ms-1">@if(isset($cmspagesUnapprovedCount)){{$cmspagesUnapprovedCount}}@endif</span></a>
                            </li>
                        </ul>
                    </div>
                </div>    
        ...
        <div class="col-12">
            <div class="card inBorderColor1">
                ...
                <div class="card-body inAddDetailTable table-responsive">
                    <form action="{{ route('admin.cmsStatus') }}" method="post" id="approveForm">
                        @csrf
                        @method('PATCH')
                        <table id="all_cms" class="table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>CMS ID</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="font-14">@if(isset($data->id)){{$data->id}}@endif</td>
                                    <td>@if($data->status == "APPROVED")<i class="fas fa-thumbs-up"></i>@else<i class="fas fa-thumbs-down pe-1"></i>@endif</td>
                                    <td>@if(isset($data->page_placement)){{$data->page_placement}}@endif</td>
                                    <td>@if(isset($data->cms_title)){{$data->cms_title}}@endif</td>
                                    <td>
                                    <a href="" class="btn btnPrimary me-1" data-bs-toggle="modal" data-bs-target="#religionModal{{$data->id}}">View Content</a> 
                                    </td>
                                    <td>
                                        <a href="{{route('admin.cmsEdit',$data->id)}}" class="btn btnPrimary me-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit"><i class="fas fa-pen"></i></a>
                                        <a href="{{route('admin.cmsDelete',$data->id)}}" class="btn btnSecondary" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Delete" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title" id="religionModalLabel">@if(isset($data->cms_title)){{$data->cms_title}}@endif</h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="row">
                                                    <div class="col-12 mb-3">
                                                        <p>@if(isset($data->cms_content)){!! $data->cms_content !!}@endif</p>
                                                    </div>
                                                </div>   
                                            </div>
                                        </div>
