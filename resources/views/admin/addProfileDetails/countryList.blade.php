...
<!-- Page title -->
@section('title') Admin - All Countries @endsection
...
<div class="container pt-3">
    ...
    <h3 class="colorSecondary inATitle1">Country</h3>
    ...
    <div class="inMemberTopPanel">
        <div class="pb-0">
            <div class="row">
                <div class="mb-3">
                    <a href="{{ route('admin.countryList') }}" class="btn btnSecondary" data-bs-toggle="modal" data-bs-target="#countryModal">
                        <i class="fas fa-plus pe-1"></i>Add Country
                    </a>
                    <div class="dropdown float-end ms-2 me-2">
                        ...
                        <ul class="dropdown-menu inBorderColor1">
                            <li>
                                <a href="{{ route('admin.countryList',['filter' => 'all']) }}" name="filter" value="all" class="dropdown-item">All Country<span class="badge text-bg-secondary ms-1">{{ $countryCount }}</span></a>
                            </li>
                            <li>
                                <a href="{{ route('admin.countryList',['filter' => 'approved']) }}" name="filter" value="approved" class="dropdown-item">Approved<span class="badge text-bg-secondary ms-1">{{ $countryApprovedCount }}</span></a>
                            </li>
                            <li>
                                <a href="{{ route('admin.countryList',['filter' => 'unapproved']) }}" name="filter" value="unapproved" class="dropdown-item">Unapproved<span class="badge text-bg-secondary ms-1">{{ $countryUnapprovedCount }}</span></a>
                            </li>
                        </ul>
                    </div>
                </div>    
        ...
        <div class="col-12">
            <div class="card inBorderColor1">
                ...
                <div class="card-body inAddDetailTable ">
                    <form action="{{ route('admin.countryStatus') }}" method="post" id="approveForm">
                        @csrf
                        @method('PATCH')
                        <table id="all_countries" class="table table-responsive">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Country Code</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="font-14">{{$data->country_code}}</td>
                                    <td class="font-14">{{$data->country_name}}</td>
                                    <td>@if($data->status == "APPROVED")<i class="fas fa-thumbs-up"></i>@else<i class="fas fa-thumbs-down pe-1"></i>@endif</td>
                                    <td>
                                        <a href="#" class="btn btnPrimary me-1" data-bs-toggle="modal" data-bs-target="#countryModal{{$data->id}}" data-bs-title="Edit">
                                            <i class="fas fa-pen"></i>
                                        </a>
                                        <a href="{{ route('admin.countryDelete',$data->id) }}" class="btn btnSecondary" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            
                                <div class="modal fade inAddDetailsModal" id="countryModal{{$data->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form method="Post" action="{{ route('admin.countryStatus') }}">
                                                @csrf
                                                @method('PATCH')
                                                <div class="modal-header">
                                                    <h1 class="modal-title" id="religionModalLabel">Edit Country</h1>
                                                ...
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-12 mb-3 mt-2">
                                                            ...
                                                            <input type="text" name="country_code" value="@if(isset($data)){{$data->country_code}}@endif"class="form-control" placeholder="Enter Country COde" required>
                                                        </div>
                                                        <div class="col-12 mb-3 mt-2">
                                                            <label class="label-1 fw-semibold">Country Name</label>
                                                            <input type="text" name="country_name" value="@if(isset($data)){{$data->country_name}}@endif" class="form-control" placeholder="Enter Country Name" required>
                                                        </div>
...
<!-- Add country modal -->
<div class="modal fade inAddDetailsModal" id="countryModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" action="{{ route('admin.countryStore') }}">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title" id="religionModalLabel">Add Country</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
