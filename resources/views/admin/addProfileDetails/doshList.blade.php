...
<!-- Page title -->
@section('title') Admin - All Dosh @endsection
...
<div class="container pt-3">
    <h3 class="colorSecondary inATitle1">Dosh</h3>
    ...
    <div class="inMemberTopPanel">
        <div class="pb-0">
            <div class="row">
                <div class="mb-3">
                    <a href="#" class="btn btnSecondary" data-bs-toggle="modal" data-bs-target="#doshModal">
                        <i class="fas fa-plus pe-1"></i>Add Dosh
                    </a>
                    <div class="dropdown float-end ms-2 me-2">
                        ...
                        <ul class="dropdown-menu inBorderColor1">
                            <li>
                                <a href="{{route('admin.doshList',['filter' => 'all'])}}" name="filter" value="all" class="dropdown-item">All Dosh<span class="badge text-bg-secondary ms-1">{{$doshCount}}</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.doshList',['filter' => 'approved'])}}" name="filter" value="approved" class="dropdown-item">Approved<span class="badge text-bg-secondary ms-1">{{$doshApprovedCount}}</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.doshList',['filter' => 'unapproved'])}}" name="filter" value="unapproved" class="dropdown-item">Unapproved<span class="badge text-bg-secondary ms-1">{{$doshUnapprovedCount}}</span></a>
                            </li>
                        </ul>
                    </div>
                </div>    
        ...
        <div class="col-12">
            <div class="card inBorderColor1">
                ...
                <div class="card-body inAddDetailTable ">
                    <form action="{{ route('admin.doshStatus') }}" method="post" id="approveForm">
                        @csrf
                        @method('PATCH')
                        <table id="all_dosh" class="table table-responsive">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
                            ...
                            <tbody>
                                @foreach ($dosh as $data)
                                <tr>
                                    <td><input type="checkbox" value="{{$data->id}}" name="selectedreligion[]" name="checkbox" id="checkbox" class="checkbox"></td>
                                    <td class="font-14">{{$data->dosh}}</td>
                                    <td>@if($data->status == "APPROVED")<i class="fas fa-thumbs-up"></i>@else<i class="fas fa-thumbs-down pe-1"></i>@endif</td>
                                    <td>
                                        <a href="#" class="btn btnPrimary me-1" data-bs-toggle="modal" data-bs-target="#doshModal{{$data->id}}" data-bs-title="Edit">
                                            <i class="fas fa-pen"></i>
                                        </a>
                                        <a href="{{route('admin.doshDelete',$data->id)}}" class="btn btnSecondary" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            
                                <div class="modal fade inAddDetailsModal" id="doshModal{{$data->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form method="Post" action="{{route('admin.doshStatus')}}">
                                                @csrf
                                                @method('PATCH')
                                                <div class="modal-header">
                                                    <h1 class="modal-title" id="religionModalLabel">Edit Dosh</h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-12 mb-3 mt-2">
                                                            <label class="label-1 fw-semibold">Dosh</label>
                                                            <input type="text" class="form-control" value="{{$data->dosh}}" name="dosh" placeholder="Enter dosh Name" required>
                                                        </div>
...
<!-- Add dosh modal -->
<div class="modal fade inAddDetailsModal" id="doshModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="Post" action="{{route('admin.doshStore') }}">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title" id="religionModalLabel">Add Dosh</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12 mb-3 mt-2">
                            <label class="label-1 fw-semibold">Dosh</label>
                            <input type="text" class="form-control" name="dosh" placeholder="Enter dosh Name" required>
                        </div>
                        <div class="col-12 mb-3">
                            <div class="row">
                                <div class="col-xl-2 col-3">
