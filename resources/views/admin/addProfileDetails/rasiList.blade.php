...
<!-- Page title -->
@section('title') Admin - All Rasi @endsection
...
<div class="container pt-3">
    <h3 class="colorSecondary inATitle1">Rasi (Moonsign)</h3>
    ...
    <div class="inMemberTopPanel">
        <div class="pb-0">
            <div class="row">
                <div class="mb-3">
                    <a href="" class="btn btnSecondary" data-bs-toggle="modal" data-bs-target="#starModal">
                        <i class="fas fa-plus pe-1"></i>Add Rasi (Moonsign)
                    </a>
                    <div class="dropdown float-end ms-2 me-2">
                        ...
                        <ul class="dropdown-menu inBorderColor1">
                            <li>
                                <a href="{{route('admin.rasiList',['filter' => 'all'])}}" name="filter" value="all" class="dropdown-item">All Rasi (Moonsign)<span class="badge text-bg-secondary ms-1">{{$rasiCount}}</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.rasiList',['filter' => 'approved'])}}" name="filter" value="approved" class="dropdown-item">Approved<span class="badge text-bg-secondary ms-1">{{$rasiApprovedCount}}</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.rasiList',['filter' => 'unapproved'])}}" name="filter" value="unapproved" class="dropdown-item">Unapproved<span class="badge text-bg-secondary ms-1">{{$rasiUnapprovedCount}}</span></a>
                            </li>
                        </ul>
                    </div>
                </div>    
        ...
        <div class="col-12">
            <div class="card inBorderColor1">
                ...
                <div class="card-body inAddDetailTable ">
                    <form action="{{ route('admin.rasiStatus') }}" method="post" id="approveForm">
                        @csrf
                        @method('PATCH')
                        <table id="all_rasi" class="table table-responsive">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
                            ...
                            <tbody>
                                @foreach ($rasi as $data)
                                <tr>
                                    <td><input type="checkbox" value="{{$data->id}}" name="selectedreligion[]" name="checkbox" id="checkbox" class="checkbox"></td>
                                    <td class="font-14">{{$data->rasi}}</td>
                                    <td>@if($data->status == "APPROVED")<i class="fas fa-thumbs-up"></i>@else<i class="fas fa-thumbs-down pe-1"></i>@endif</td>
                                    <td>
                                        <a href="#" class="btn btnPrimary me-1" data-bs-toggle="modal" data-bs-target="#rasiModal{{$data->id}}" data-bs-title="Edit">
                                            <i class="fas fa-pen"></i>
                                        </a>
                                        <a href="{{route('admin.rasiDelete',$data->id)}}" class="btn btnSecondary" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            
                                <div class="modal fade inAddDetailsModal" id="rasiModal{{$data->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form method="Post" action="{{ route('admin.rasiStatus') }}">
                                                @csrf
                                                @method('PATCH')
                                                <div class="modal-header">
                                                    <h1 class="modal-title" id="religionModalLabel">Edit Rasi (Moonsign)</h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-12 mb-3 mt-2">
                                                            <label class="label-1 fw-semibold">Rasi (Moonsign) Name</label>
                                                            <input type="text" class="form-control" value="{{$data->rasi}}" name="rasi" placeholder="Enter rasi Name" required>
                                                        </div>
...
<!-- /.Main Content -->
<!-- Add rasi modal -->
<div class="modal fade inAddDetailsModal" id="starModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="Post" action="{{route('admin.rasiStore') }}">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title" id="religionModalLabel">Add Rasi (Moonsign)</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12 mb-3 mt-2">
                            <label class="label-1 fw-semibold">Rasi (Moonsign) Name</label>
                            <input type="text" class="form-control" name="rasi" placeholder="Enter rasi Name" required>
                        </div>
                        <div class="col-12 mb-3">
                            <div class="row">
                                <div class="col-xl-2 col-3">
