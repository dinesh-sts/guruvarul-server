...
<!-- Page title -->
@section('title') Admin - All Subcaste @endsection
...
<div class="container pt-3">
    ...
    <h3 class="colorSecondary inATitle1">Sub Caste</h3>
    ...
    <div class="inMemberTopPanel">
        <div class="pb-0">
            <div class="row">
                <div class="mb-3">
                    <a href="" class="btn btnSecondary" data-bs-toggle="modal" data-bs-target="#subcasteModal">
                        <i class="fas fa-plus pe-1"></i>Add Sub Caste
                    </a>
                    <div class="dropdown float-end ms-2 me-2">
                        ...
                        <ul class="dropdown-menu inBorderColor1">
                            <li>
                                <a href="{{route('admin.subcasteList',['filter' => 'all'])}}" name="filter" value="all" class="dropdown-item">All Sub Caste<span class="badge text-bg-secondary ms-1">{{ $subcasteCount }}</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.subcasteList',['filter' => 'approved'])}}" name="filter" value="approved" class="dropdown-item">Approved<span class="badge text-bg-secondary ms-1">{{ $subcasteApprovedCount }}</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.subcasteList',['filter' => 'unapproved'])}}" name="filter" value="unapproved" class="dropdown-item">Unapproved<span class="badge text-bg-secondary ms-1">{{ $subcasteUnapprovedCount }}</span></a>
                            </li>
                        </ul>
                    </div>
                </div>    
        ...
        <div class="col-12">
            <div class="card inBorderColor1">
                ...
                <div class="card-body inAddDetailTable">
                    <form action="{{ route('admin.subcasteStatus') }}" method="post" id="approveForm">
                        @csrf
                        @method('PATCH')

                        <table id="all_subcaste" class="table table-responsive">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
                            ...
                            <tbody>
                                @foreach ($subcaste as $data)
                                <tr>
                                    <td><input type="checkbox" value="{{$data->id}}" name="selected[]" name="checkbox" id="checkbox" class="checkbox"></td>
                                    <td class="font-14">{{$data->sub_caste_name}}</td>
                                    <td>@if($data->status == "APPROVED")<i class="fas fa-thumbs-up"></i>@else<i class="fas fa-thumbs-down pe-1"></i>@endif</td>
                                    <td>
                                        <a href="#" class="btn btnPrimary me-1" data-bs-toggle="modal" data-bs-target="#subcasteModal{{$data->id}}" data-bs-title="Edit">
                                            <i class="fas fa-pen"></i>
                                        </a>
                                        <a href="{{ route('admin.subcasteDelete',$data->id) }}" class="btn btnSecondary" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                            
                                <div class="modal fade inAddDetailsModal" id="subcasteModal{{$data->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form method="post" action="{{ route('admin.subcasteStatus') }}">
                                                @csrf
                                                @method('PATCH')
                                                <div class="modal-header">
                                                    <h1 class="modal-title" id="religionModalLabel">Edit Sub Caste</h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-12 mb-3 mt-2">
                                                            <label class="label-1 fw-semibold">Sub Caste Name</label>
                                                            <input type="text" value="@if(isset($data)){{ $data->sub_caste_name }}@endif"  name="sub_caste_name" class="form-control" placeholder="Enter Sub Caste Name" required>
                                                        </div>
...
<div class="modal fade inAddDetailsModal" id="subcasteModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" action="{{route('admin.subcasteStore') }}">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title" id="religionModalLabel">Add Sub Caste</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12 mb-3 mt-2">
                            <label class="label-1 fw-semibold">Sub Caste Name</label>
                            <input type="text" name="sub_caste_name" class="form-control" placeholder="Enter Sub Caste Name" required>
                        </div>
                        <div class="col-12 mb-3">
                            <div class="row">
                                <div class="col-xl-2 col-3">
