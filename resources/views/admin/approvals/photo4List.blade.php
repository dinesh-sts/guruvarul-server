...
<!-- Page title -->
@section('title') Admin - Photo 4 Approval @endsection
...
<div class="container pt-3">
    ...
    <h3 class="colorSecondary inATitle1">Photo 4 Approval</h3>
                ...
                <div class="mb-3">
                    <div class="dropdown d-inline float-end ms-2 me-2">
                        ...
                        <ul class="dropdown-menu inBorderColor1">
                            <li>
                                <a href="{{route('admin.photo4List',['filter' => 'all'])}}" name="filter" value="all" class="dropdown-item">All Photo 4<span class="badge text-bg-secondary ms-1">@if(isset($photo4Count)){{$photo4Count}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.photo4List',['filter' => 'approved'])}}" name="filter" value="approved" class="dropdown-item">Approved<span class="badge text-bg-secondary ms-1">@if(isset($photo4ApprovedCount)){{$photo4ApprovedCount}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.photo4List',['filter' => 'unapproved'])}}" name="filter" value="unapproved" class="dropdown-item">Unapproved<span class="badge text-bg-secondary ms-1">@if(isset($photo4UnapprovedCount)){{$photo4UnapprovedCount}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.photo4List',['filter' => 'pending'])}}" name="filter" value="pending" class="dropdown-item">Pending<span class="badge text-bg-secondary ms-1">@if(isset($photo4PendingCount)){{$photo4PendingCount}}@endif</span></a>
                            </li>
                        </ul>
                    </div>
                </div>    
        ...
        <div class="col-12">
            <div class="card inBorderColor1">
                ...
                <div class="card-body inAddDetailTable table-responsive">
                    <form action="{{ route('admin.photo4Status') }}" method="post" id="approveForm">
                        @csrf
                        @method('PATCH')
                        <table id="all_photo4" class="table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>@if($data->photo4_approve == "APPROVED")<i class="fas fa-thumbs-up"></i>@elseif($data->photo4_approve == "UNAPPROVED")<i class="fas fa-thumbs-down pe-1"></i>@else <i class="fas fa-clock pe-1"></i> @endif</td>
                                    <td>  
                                        <a href="" data-bs-toggle="modal" data-bs-target="#photoModal{{$data->id}}">
                                        <?php  $filePath = '/userImages/'.$data->photo4; ?>
                                        @if($data->photo4 != "" && Storage::disk('public')->exists($filePath))
                                        <img src="{{asset('storage/userImages/'.$data->photo4)}}" class="img-thumbnail maxH-60">
                                        @endif
                                    
                                    </a>
                                    </td>
                                    <td>
                                        <a href="{{route('admin.photo4Delete',$data->id)}}" class="btn btnSecondary" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Delete" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                                ...
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-12 mb-3 mt-2">
                                                            <?php  $filePath = '/userImages/'.$data->photo4; ?>
                                                            @if($data->photo4 != "" && Storage::disk('public')->exists($filePath))
                                                                <img src="{{asset('storage/userImages/'.$data->photo4)}}" class="img-fluid w-100">
                                                            @endif
