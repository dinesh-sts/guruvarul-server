...
<!-- Page title -->
@section('title') Admin - Horoscope Approval @endsection
...
<div class="container pt-3">
    ...
    <h3 class="colorSecondary inATitle1">Horoscope Approval</h3>
                ...
                <div class="mb-3">
                    <div class="dropdown d-inline float-end ms-2 me-2">
                        ...
                        <ul class="dropdown-menu inBorderColor1">
                            <li>
                                <a href="{{route('admin.horoscopeList',['filter' => 'all'])}}" name="filter" value="all" class="dropdown-item">All Horoscope<span class="badge text-bg-secondary ms-1">@if(isset($horoscopeCount)){{$horoscopeCount}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.horoscopeList',['filter' => 'approved'])}}" name="filter" value="approved" class="dropdown-item">Approved<span class="badge text-bg-secondary ms-1">@if(isset($horoscopeApprovedCount)){{$horoscopeApprovedCount}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.horoscopeList',['filter' => 'unapproved'])}}" name="filter" value="unapproved" class="dropdown-item">Unapproved<span class="badge text-bg-secondary ms-1">@if(isset($horoscopeUnapprovedCount)){{$horoscopeUnapprovedCount}}@endif</span></a>
                            </li>
                            <li>
                                <a href="{{route('admin.horoscopeList',['filter' => 'pending'])}}" name="filter" value="pending" class="dropdown-item">Pending<span class="badge text-bg-secondary ms-1">@if(isset($horoscopePendingCount)){{$horoscopePendingCount}}@endif</span></a>
                            </li>
                        </ul>
                    </div>
                </div>    
        ...
        <div class="col-12">
            <div class="card inBorderColor1">
                ...
                <div class="card-body inAddDetailTable table-responsive">
                    <form action="{{ route('admin.horoscopeStatus') }}" method="post" id="approveForm">
                        @csrf
                        @method('PATCH')
                        <table id="all_horoscope" class="table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Name</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>@if($data->hor_check == "APPROVED")<i class="fas fa-thumbs-up"></i>@elseif($data->hor_check == "UNAPPROVED")<i class="fas fa-thumbs-down pe-1"></i>@else <i class="fas fa-clock pe-1"></i> @endif</td>
                                    <td>  
                                        <a href="#" data-bs-toggle="modal" data-bs-target="#horModal{{$data->id}}">
                                            <?php  $filePath = '/userImages/'.$data->hor_photo; ?>
                                            @if($data->hor_photo != "" && Storage::disk('public')->exists($filePath))
                                                <img src="{{asset('storage/userImages/'.$data->hor_photo)}}" class="img-thumbnail maxH-60">
                                            @endif
                                        </a>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.horoscopeDelete',$data->id) }}" class="btn btnSecondary" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Delete" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                <div class="modal fade inAddDetailsModal" id="horModal{{$data->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="religionModalLabel" aria-hidden="true">
                                                ...
                                                <div class="modal-body">
                                                    <div class="row">
                                                        <div class="col-12 mb-3 mt-2">
                                                            <?php  $filePath = '/userImages/'.$data->hor_photo; ?>
                                                            @if($data->hor_photo != "" && Storage::disk('public')->exists($filePath))
                                                            <img src="{{asset('storage/userImages/'.$data->hor_photo)}}" class="img-fluid w-100">
                                                            @endif
                                                        </div>  
                                                    </div>   
                                                </div>
