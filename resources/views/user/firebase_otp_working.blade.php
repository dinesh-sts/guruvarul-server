...
<script type="module">
	...
	confirmationResult.confirm(code)
	    .then((result) => {
	        ...
	        statusEl.textContent = "✅ OTP Verified! User ID: " + firebaseUser.uid;
	        statusEl.className = "success";

	        console.log("Firebase User Object:", firebaseUser);

	        // Clean phone number (remove +91)
	        let cleanPhone = firebaseUser.phoneNumber.replace("+91", "").trim();

	        // Send to Laravel for actual login
	        fetch("{{ route('user.firebaseLogin') }}", {
	            method: "POST",
	            headers: {
