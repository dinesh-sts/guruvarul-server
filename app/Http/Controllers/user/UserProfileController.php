...
class UserProfileController extends Controller
{
	...
	public function deleteProfileStore(Request $request)
	{
		...
		// Save to delete_profiles table
		$data = new DeleteProfile();
		$data->matri_id = $id->matri_id;
		$data->reason = $request->reason;
		$data->donate = $request->has('donate') ? 1 : 0;

		if ($request->hasFile('payment_screenshot')) {
		$path = $request->file('payment_screenshot')->store('donations', 'public');
		$data->payment_screenshot = $path;

		// 🔹 Also update in registers table
		\App\Models\Register::where('matri_id', $id->matri_id)
		    ->update(['donation_image' => $path]);
		}

		$data->save();

		return redirect()->route('user.dashboard')
		->with('message', 'Profile delete request sent successfully.');
	}
