...
class RegisterController extends Controller
{
    ...
    public function registerMobileVerify(Request $request)
    {
        ...
        if ($user) {
            ...
            // Check which SMS API is active
            $activeapi = Sms::where('key', 'activeapi')->first();

            // Send OTP only if using Fast2SMS
            if ($activeapi && $activeapi->value != 'firebase') {
                $this->smsapi($otp, $mobile);
            }

            // Load correct Blade file
            if ($activeapi && $activeapi->value == 'firebase') {
                return view('user.registerMobileVerify_firebase', compact('id'));
            } else {
                return view('user.registerMobileVerify', compact('id'));
            }
        } else {
...
/*public function registerOtpVerify(Request $request)
{
    ...
    try {
        // Get phone and Firebase UID from request
        $phone = preg_replace('/[^0-9]/', '', $request->phone); // Only digits
        $firebaseUid = $request->uid ?? null;
        ...
        // Find the user in registers table
        $register = Register::where('mobile', $phone)->first();
        ...
        // Clear any session OTP data
        Session::forget('otp');
        Session::forget('user_id');

        // Determine redirect
        if ($siteconfig->registerPersonalDetails === "Yes") {
            $redirect = route('user.registerPersonalDetails');
        } elseif ($siteconfig->registerPreferenceDetails === "Yes") {
            $redirect = route('user.registerPersonalDetails');
        } else {
            //$redirect = route('user.registerDocumentUpload');
	    $redirect = route('user.registerConfirmation');
        }
    ...
    public function registerPersonalDetailsPost(Request $request){
	    ...
	    //return redirect()->route('user.registerDocumentUpload')->with('message', 'Data Updated Sucessfully');
	    return redirect()->route('user.registerConfirmation');
        }else{
            //return redirect()->route('user.registerDocumentUpload');
	    return redirect()->route('user.registerConfirmation');
