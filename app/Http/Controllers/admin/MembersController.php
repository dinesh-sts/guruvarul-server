...
class MembersController extends Controller{
	...
	/*if ($filter === 'expired') {
	    // get latest payments per user
	    $latestPayments = Payment::select('id', 'pmatri_id', 'exp_date')
	    ...
	    foreach ($latestPayments as $payment) {
	        $expDate = Carbon::parse($payment->exp_date);
	        if ($expDate->lessThanOrEqualTo(Carbon::today())) {
	            $expiredMatriIds[] = $payment->pmatri_id;
	        }
	    }

	    // filter members by expired matri_ids
	    $query->whereIn('matri_id', $expiredMatriIds);
	}*/
	...
	// Expired Users Count.
	/*$expiredMembersCount = Register::whereExists(function ($query) {
        $query->select(FacadesDB::raw(1))
	            ->from('payments')
	            ->whereColumn('payments.pmatri_id', 'registers.matri_id')
	            ->where('payments.exp_date', '<', Carbon::today());
	    })->count();*/
        ...
        //return view('admin.members.allMembers',compact('members','ages','castes','countries','religions','allMembersCount','featuredMembersCount','unapprovedMembersCount','approvedMembersCount','paidMembersCount','expiredMembersCount'));
	return view('admin.members.allMembers', compact(
	    'members',
	    'ages',
	    'castes',
	    'countries',
    ...
    public function membersApprovedToPaid(Request $request)
    {
        ...
        $query = Register::whereIn('status', ['Active', 'Expired']) // ✅ allow both Paid
            ->whereNotNull('payment_image')                      // ✅ must have payment proof
            ->select(
                'id','photo1_approve','matri_id', 'firstname', 'lastname',
                'profileby', 'email', 'gender', 'height', 'caste',
                'mobile', 'birthdate', 'religion', 'address', 'photo1',
                'country_id','state_id','occupation','m_tongue','status','fstatus'
            )
            ->selectRaw('YEAR(CURDATE()) - YEAR(birthdate) - (DATE_FORMAT(CURDATE(), "%m%d") < DATE_FORMAT(birthdate, "%m%d")) AS age');

        $members = $query->orderByDesc('id')->paginate(25);

        // Counts (these still include all users, not just with payment_image)
        $allMembersCount = Register::count();        
        $featuredMembersCount = Register::where('fstatus',"Featured")->count();
        $unapprovedMembersCount = Register::where('status',"Inactive")->count();
        $approvedMembersCount = Register::where('status',"Active")->count();
	...
	$query->select(FacadesDB::raw(1))
        ...
        return view('admin.members.membersApprovedToPaid', compact(
            'allMembersCount',
            'featuredMembersCount',
            'unapprovedMembersCount',
            'approvedMembersCount',
            'paidMembersCount',
            'ages','castes','countries','religions',
	...
	$query->select(FacadesDB::raw(1))
        ...
        $renewMembers = $query->orderByDesc('id')->paginate(20);
    ...
    public function unpaidMembers(Request $request)
    {
        ...
        // Get all matri_ids present in the payments table
        $paidMatriIds = Payment::pluck('pmatri_id')->toArray();

        // Query members NOT in payments table
        $query = Register::whereNotIn('matri_id', $paidMatriIds)
            ->select(
                'id', 'matri_id', 'firstname', 'lastname', 'profileby', 'email',
                'gender', 'height', 'caste', 'mobile', 'birthdate', 'religion',
                'address', 'photo1', 'country_id', 'state_id', 'occupation',
                'm_tongue', 'status', 'fstatus', 'photo1_approve'
            )
            ->with('hei', 'rel', 'occ', 'mother_tongue', 'h_edu', 'country', 'state')
        ...
        // Paginate results
        $unpaidMembers = $query->orderByDesc('id')->paginate(20);
        ...
        return view('admin.members.unpaidMembers', compact(
            'allMembersCount',
            'featuredMembersCount',
            'unapprovedMembersCount',
            'approvedMembersCount',
    ...
    {
        try {
            // Find the member in the registers table
            $data = Register::findOrFail($id);

            // Insert into delete_profiles table
            DeleteProfile::create([
                'matri_id' => $data->matri_id,
                'reason' => 'Suspended by Admin',  // You can make this dynamic if needed
                'donate' => 1,                  // Default value, change if you have data
                'payment_screenshot' => null,      // Default value, change if you have data
                'status' => 'Yes',           // Status for delete_profiles
                'created_at' => now(),
                'updated_at' => now(),
            ]);

            // Update status in registers table instead of deleting
            $data->status = 'Suspended';
            $data->save();

            return redirect()->route('admin.membersAll')->with('message', 'Member Suspended and record moved to delete_profiles successfully');

        } catch (QueryException $e) {
            return redirect()->back()->with('message', 'An error occurred while processing the member.');
        }
    }
